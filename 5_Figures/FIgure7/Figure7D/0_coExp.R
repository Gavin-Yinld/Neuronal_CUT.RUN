...
load("matrix.Rdata")
EXC.genes <- read.table("EXC.genes.unique",header=F)[,1]
...
rownames(mat) <- mat[,1]
mat <- mat[,-1]  
genes.exp <- mat[which(rownames(mat) %in% EXC.genes),] 
...
cl <- mfuzz(df3F,c=5,m=m)
mfuzz.plot2(df3F, cl=cl,mfrow=c(3,3),centre=TRUE,x11=F,centre.lwd=0.2)
save(cl,file="EXC.cluster.result.Rdata")
...
cor.egr1 <- c()
Egr1.exp <- EXC_mat[grep("Egr1",rownames(EXC_mat)),]
for(i in 1:max(cl$cluster))
{
  genes <- names(cl$cluster)[which(cl$cluster==i)]
  genes.exp2 <- genes.exp[genes,grep("EXC",colnames(genes.exp))]
  genes.exp2 <- colMeans(genes.exp2)
  cor.egr1 <- c(cor.egr1,cor(t(Egr1.exp),as.numeric(genes.exp2)))
}

save(cor.egr1,file="EXC.cluster.correlation.with.Egr1.Rdata")
...
#pdf("EXC.all.cluster.pdf",height=15,width=10)
...
write.table(genes,file="EXC.module1.genes",quote=F,row.names=F,col.names=F)
